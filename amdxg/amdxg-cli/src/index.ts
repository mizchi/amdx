...
function buildRSS(flags: {}) {
  ...
  <rss version="2.0">
    <channel>
      <title>${escape(amdxgConfig.siteName)}</title>
      <link>${amdxgConfig.host}</link>
      <description></description>
      <language>${amdxgConfig.lang}</language>
      ...
      ${pages
        ...
        <item>
          ...
          <link>${amdxgConfig.host + "/" + p.slug}</link>
  ...
  console.log("[amdxg:postbuild:rss] out/rss.xml");
}
...
function buildIndex(flags: {}) {
  ...
  console.log("[amdxg:build:index]", "gen/pages.json");
}
...
function buildHistory(flags: {}) {
  genPages(process.cwd()).forEach((page) => {
    ...
    console.log("[amdxg:build:history]", `gen/${page.slug}.history.json`);
  });
}
